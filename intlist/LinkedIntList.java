...
public class LinkedIntList {
    ...
    public static LinkedIntList concatenated(LinkedIntList A, LinkedIntList B) {
        if (A.front == null) {
            return new LinkedIntList(recursiveConcatenatedHelper(B.front));
        } else if (B.front == null) {
            return new LinkedIntList(recursiveConcatenatedHelper(A.front));
        } else {
            ListNode result = recursiveConcatenatedHelper(A.front);
            ListNode p = result;
            while (p.next != null) {
                p = p.next;
            }
            p.next = recursiveConcatenatedHelper(B.front);
            return new LinkedIntList(result);
        }
    }

    private static ListNode recursiveConcatenatedHelper(ListNode p) {
        if (p == null) {
            return null;
        } else {
            ListNode concatenatedRest = recursiveConcatenatedHelper(p.next);
            return new ListNode(p.data, concatenatedRest);
        }
    }
